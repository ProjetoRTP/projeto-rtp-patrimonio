  ...
  btnConfirmarEnvio.addEventListener("click", async () => {
    ...
    try {
      ...
      const barcodeResponse = await fetch(
        `${API_URL}/api/equipamentos/${equipamentoId}/barcode`,
        {
          headers: getHeaders(),
        },
      );
      ...
      const barcodeJson = await barcodeResponse.json();
      const generatedBarcode =
        barcodeJson.barcode || barcodeJson.data?.barcode || currentBarcode;
      ...
      const telegramResponse = await fetch(
        `${API_URL}/api/notificacoes/telegram`,
        {
          method: "POST",
          headers: getHeaders(),
